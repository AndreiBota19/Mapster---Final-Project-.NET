...
public readonly ref struct MapFeatureData
{
    ...
    public Dictionary<TypeOfRelief, string> Properties { get; init; }
}
    ...
    public void ForeachFeature(BoundingBox b, MapFeatureDelegate? action)
    {
        ...
        for (var i = 0; i < tiles.Length; ++i)
        {
            ...
            for (var j = 0; j < header.Tile.Value.FeaturesCount; ++j)
            {
                ...
                if (isFeatureInBBox)
                {
                    var properties = new Dictionary<TypeOfRelief, string>(feature->PropertyCount);
                    for (var p = 0; p < feature->PropertyCount; ++p)
                    {
                        ...
                        if (!types.Contains(key.ToString())) continue;
                        try
                        {
                            TypeOfRelief typeOfRelief = (TypeOfRelief)Enum.Parse(typeof(TypeOfRelief), key.ToString());
                            properties.Add(typeOfRelief, value.ToString());  
                        } catch (Exception e)
                        {
                            continue;
                        }
